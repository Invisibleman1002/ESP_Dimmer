 ...
 * When you turn off the switch, the device is ran on battery, so it goes into LIGHT_SLEEP mode.
 ...
 * .
 * home/masterbath/light/getldr - send any message to that topic and you will get a message(topic ldr) back telling you the LDR value, can be used to set your threshold. !Make sure you send a value, otherwise is crashes it.
 *
...
void onMqttMessage(char *topic, char *payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total)
{
  ...
  { // getldr
    ...
    DEBUG_ERROR_SERIAL.print("  fulltopic: ");
    DEBUG_ERROR_SERIAL.println(sendtopc);
    char buf[4];
  ...
  { // light
    ...
    DEBUG_ERROR_SERIAL.print("  lights: ");
    DEBUG_ERROR_SERIAL.println(lights);
...
void loop()
{
  ...
  if (PoweredPinstate == LOW && millis() - timelastslept > 60000)
  { // 1 hour 3600000000
  ...
  if ((millis() - lastDebounceTime) > debounceDelay)
  {
    ...
    if (reading != buttonState)
    {
      ...
      if (buttonState == HIGH)
      {
        ...
        if (!WiFi.isConnected())
        {
          DEBUG_ERROR_SERIAL.print("button on wifi status: ");
          DEBUG_ERROR_SERIAL.println(WiFi.status());
          poweredStartWifi.once(10, connectToWifi);
        }
      }
